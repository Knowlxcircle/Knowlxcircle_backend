...
class GenerateSearch(APIView):
    def post(self, request):
        try:
            ...
            prompt_response = model.generate_search(search_query)
            prompt = Prompt.objects.create(prompt=search_query)
            response_object = GeminiResponse.objects.create(prompt=prompt, response=prompt_response)
            # response_serializer = GeminiResponseSerializer(response_object)
            data["prompt"] = prompt.prompt
            data["response"] = prompt_response
